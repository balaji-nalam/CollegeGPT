...
import { Lock, Mail, User, Building, ArrowRight, Loader2, Eye, EyeOff, ShieldCheck } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await register(formData.name, formData.email, formData.password, formData.department);
    if (res.success) {
  ...
  return (
    <>
      <Head>
        <title>Student Registration | CollegeGPT</title>
      </Head>
          ...
          <div className="relative grid w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-800 bg-[#0d131f]/88 shadow-2xl backdrop-blur-xl md:grid-cols-[.9fr_1.1fr]">
            <div className="hidden flex-col justify-between bg-gradient-to-br from-indigo-600/25 via-slate-900 to-cyan-500/10 p-10 md:flex">
              <div>
                ...
                <h1 className="mt-6 text-3xl font-bold text-white">Your academic answers, verified.</h1>
                <p className="mt-3 text-sm leading-relaxed text-slate-300">Create a student account to consult official college documents with confidence.</p>
              </div>
              <p className="flex items-center gap-2 text-xs text-slate-300"><ShieldCheck className="h-4 w-4 text-emerald-400" /> Private, role-based access</p>
            </div>
            <div className="p-8">
              {/* Header */}
              <div className="flex flex-col items-center text-center">
                ...
                <h2 className="mt-4 text-2xl font-bold tracking-tight text-white">Student Registration</h2>
                <p className="mt-1 text-sm text-slate-400">Create your official CollegeGPT student account</p>
              </div>
